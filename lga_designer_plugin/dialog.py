...
class LGADesignerDialog(wx.Dialog):
    ...
    def OnGenerate(self, event):
        ...
        # -- Save .kicad_mod to user library dir --------------------------
        user_lib = os.path.join(
            pcbnew.SETTINGS_MANAGER.GetUserSettingsPath(),
            "footprints", "LGA_Designer.pretty"
        )
        os.makedirs(user_lib, exist_ok=True)

        out_path = os.path.join(user_lib, "{}.kicad_mod".format(name))
        pcbnew.FootprintSave(user_lib, fp)

        wx.MessageBox(
            "Footprint saved to:\n{}\n\n"
            "Active pads: {}\n"
            "Library: LGA_Designer.pretty\n\n"
            "Add the library in KiCad:\n"
            "PCB Editor > Preferences > Manage Footprint Libraries".format(
                out_path, len(active)),
            "LGA Designer – Done",
